<?php include("header.php"); ?>

			<div id="main-nav" class="row">
				<ul>
					<li id="prds-pop"><a href="javascript:;" class="all-prds">所有商品分类</a></li>
					<li><a href="index.php" >首页</a></li>
					<li><a href="innovate.php" class="active">创新产品</a></li>
					<li><a href="javascript:;">流量专区</a></li>
					<li><a href="javascript:;">沃云.推荐</a></li>
					<li><a href="javascript:;">团购</a></li>
					<li><a href="javascript:;">限时购</a></li>
				</ul>
			</div>

			<div class="cont">
				<div class="helpbox">
					<div class="bread-nav">
						<span>全部结果</span>
						<i>></i>
						<span>帮助中心</span>
						<i>></i>
						<span>发票说明</span>
					</div>

					<div class="help-cont">
						<div class="helpleft">
				           	<div class="helplefttitle">帮助中心</div>
				           	<div class="helpdd">购物指南</div>
					           	<ul>
				                    <li><a href="#">购物流程</a></li>
				                    <li class="active"><a href="#" >发票说明</a></li>
				                    <li><a href="#">联系客服</a></li>
				                    <li><a href="#">注册协议</a></li>
					           	</ul> 
				           	<div class="helpdd">支付方式</div>
					           	<ul>
				                    <li><a href="#">在线支付</a></li>
					           	</ul> 
				           	<div class="helpdd">配送方式</div>
					           	<ul>
				                    <li><a href="#">快递配送</a></li>
					           	</ul> 
				           	<div class="helpdd">售后服务</div>
					           	<ul>
			                    	<li><a href="#">退换货政策</a></li>
				                    <li><a href="#" >退换货流程</a></li>
				                    <li><a href="#">退款说明</a></li>
				                    <li><a href="#">手机维修</a></li>
					           	</ul> 
				           	<div class="helpdd">联通特色服务</div>
					           	<ul>
				                    <li><a href="#">交费充值</a></li>
				                    <li><a href="#" >营业厅分布查询</a></li>
				                    <li><a href="#">号码归属地查询</a></li>
					           	</ul> 
		         		</div>

		         		<div class="helpright">
		              		<div class="helprighttitle">发票说明</div>
			              	<div class="helprightdd">
			                <p>1、联通商城所售商品（号卡、终端合约计划、上网卡等）都是正品行货，均自带发票。</p> 
			                <p>2、联通商城所售商品发票将根据您的收货地址随商品一并配送给您。</p> 
			                <p>3、联通商城销售的部分终端和配件，相关发票由供货商提供，其他产品的发票，由联通公司提供。</p>
			              </div>
			         	</div>



					</div>
				</div>	

			</div>

			<div class="side service-side none" id="side-pop">
					<?php include("service_side.php"); ?>
			</div><!--//service-side-->
		</div><!--//home-part-1-->

	</div>


<link rel="stylesheet" href="spm_modules/alice-paging/1.1.0/paging.css">
<?php include("footer.php"); ?>